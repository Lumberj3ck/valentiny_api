...
def update_section_raise_if_exception(db: Session, section: SectionSave, user: UserAuthenticate):
    try:
        crud.update_section(db, section, user)
    except NoDBInstance:
        raise HTTPException(status_code=400, detail="No section or text input or image input with given id")
...
def create_section_raise_if_exception(db: Session, section: SectionSave, user: UserAuthenticate):
    try:
